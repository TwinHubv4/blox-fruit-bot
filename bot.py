...
def find_account(search: str):
    ...
    You can type first letters or any part of the username.
    Example: god -> Godofgrandson, sau -> Sauske2418, storm -> Stormylightdancer31
    """
    con = connect()
    cur = con.cursor()
...
async def settrades(interaction: discord.Interaction, username: str, amount: int):
    reset_if_needed()
    if amount < 0:
        return await interaction.response.send_message("Error: trades cannot be negative.")
    ...
    try:
        ...
        await interaction.response.send_message(f"Trades for **{real_username}** set to **{amount}**.")
    except Exception as e:
        await interaction.response.send_message(f"Error: {e}")
...
async def setcap(interaction: discord.Interaction, username: str, cap: int):
    reset_if_needed()
    if cap <= 0:
        return await interaction.response.send_message("Error: cap must be greater than 0.")
    ...
    try:
        ...
        await interaction.response.send_message(f"Max cap for **{real_username}** updated to **{cap}**.")
    except Exception as e:
        await interaction.response.send_message(f"Error: {e}")
